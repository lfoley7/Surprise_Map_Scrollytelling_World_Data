...
military_spending_data.loc[:, 'z-score-' + yearTested] = (2 * (military_spending_data[yearTested] - min_spending) / (max_spending - min_spending)) - 1

print(len(military_spending_data['z-score-' + yearTested]))
#military_spending_data_z = (2 * (military_spending_data['1960'] - min_spending) / (max_spending - min_spending)) - 1
...
plt.scatter(military_spending_data['z-score-' + yearTested], population_data[yearTested + ' [YR' + yearTested + ']'])

for i, txt in enumerate(military_spending_data['Country']):
    plt.text(military_spending_data['z-score-' + yearTested].iloc[i], population_data[yearTested + ' [YR' + yearTested + ']'].iloc[i], txt)
